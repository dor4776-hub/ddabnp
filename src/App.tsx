...
const App: React.FC = () => {
    ...
    const loadData = async () => {
      try {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
          // מילון שמונע כפילויות - שומר רק את השמירה האחרונה של כל אירוע!
          ...
          data.forEach((row: any) => {
            ...
            if (parsedEvent && parsedEvent.id) {
              // דורס שמירות ישנות של אותו אירוע, כך שלא יווצרו כפילויות בהיסטוריה
              eventsMap.set(parsedEvent.id, parsedEvent);
            }
          });

          // הפיכת המילון בחזרה למערך ומיון לפי תאריך
  ...
  const downloadConsumptionReport = async (event: EventRecord) => {
    ...
    // נשלח לרקע בלי לתקוע את האפליקציה
  ...
  const handleSaveEvent = async (updatedEvent: EventRecord) => {
    // הברקה שלך: שומר אך ורק את הפריטים שסומנו או הוצאו!
    const filteredItems = updatedEvent.items.filter(item => item.quantityOut > 0 || item.isChecked);
    
    // אם לחצת "שמור" והכל עוד ריק, נשמור את הרשימה המלאה כדי שלא תאבד אותה בטעות
    ...
    // מחזיר אותך מיד למסך הבית - אין יותר שגיאות ותקיעות!
